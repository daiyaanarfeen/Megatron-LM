...
# Same-allocation 14-stage healthy EP32/EP32 versus NEP EP32/EP28 benchmark.
...
#SBATCH --job-name=coreai_comparch_sysarch-nep.ep32-28-scatter

set -euo pipefail

REPO_DIR="${REPO_DIR:-/home/darfeen/Megatron-LM}"
ROOT_DIR="${ROOT_DIR:-${REPO_DIR}/slurm_runs/lyris_a3b_ep32_ep28_scatter_progress_ab}"
...
RUN_HEALTHY_PROFILE="${RUN_HEALTHY_PROFILE:-0}"
RUN_NEP_PROFILE="${RUN_NEP_PROFILE:-0}"
SEQ_LENGTH="${SEQ_LENGTH:-18368}"
...
NEP_EXPERT_BUCKET_GROUPS="${NEP_EXPERT_BUCKET_GROUPS:-7}"
...
for toggle in CASE_A2A_SCATTER_SCHEDULER CASE_END_ITERATION_SCATTER; do
    case "${!toggle}" in
        0|1) ;;
        *)
            echo "${toggle} must be 0 or 1" >&2
...
if ((CASE_A2A_SCATTER_SCHEDULER && CASE_END_ITERATION_SCATTER)); then
...
if ((NUM_EXPERTS % 32 != 0 || NUM_EXPERTS % 28 != 0)); then
    echo "NUM_EXPERTS must be divisible by both EP32 and EP28" >&2
    exit 2
fi
...
srun --nodes=16 --ntasks=16 --ntasks-per-node=1 --mpi=none \
    "${container_args[@]}" \
    bash -lc "cd '${REPO_DIR}' && python -c 'import torch, mamba_ssm, causal_conv1d; print(\"[ep32-28-scatter] container warm: ok\")'"

if [[ "${RUN_CORRECTNESS}" == "1" ]]; then
    echo "[ep32-28-scatter] $(date --iso-8601=seconds) starting two-layer correctness gate"
    env \
        REPO_DIR="${REPO_DIR}" \
        ROOT_DIR="${ROOT_DIR}/correctness" \
        IMAGE="${IMAGE}" \
        CONTAINER_NAME="${CONTAINER_NAME}" \
        CASE_LABEL=ep32_ep28 \
        CASE_DISPLAY=ep32-28 \
        CASE_RUN_NNODES=15 \
        CASE_RUN_NPROC_PER_NODE=4 \
        CASE_RUN_WORLD_SIZE=60 \
        CASE_USE_DIRECT_SRUN_RANKS=1 \
        CASE_GLOBAL_BATCH_SIZE=60 \
        CASE_NUM_LAYERS=2 \
        CASE_NUM_EXPERTS=224 \
        CASE_TENSOR_MODEL_PARALLEL_SIZE=2 \
        CASE_EXPERT_MODEL_PARALLEL_SIZE=32 \
        CASE_NONUNIFORM_EP_TOPOLOGY="16 14" \
        CASE_DEBUG_RANKS="0 28 32 56" \
        CASE_EXPECTED_RANKS=60 \
        ...
        CASE_TIMEOUT=8m \
        bash "${CORRECTNESS}"
    echo "[ep32-28-scatter] $(date --iso-8601=seconds) correctness gate passed"
fi
...
run_case() {
    ...
    if [[ "${run_selected}" != "1" ]]; then
        echo "[ep32-28-scatter] skipping ${stage}/${label}"
        return
    fi
    ...
    timeout --foreground --signal=TERM --kill-after=45s "${CASE_TIMEOUT}" \
        env \
            ...
            MEGATRON_NONUNIFORM_EP_END_ITERATION_SCATTER="${CASE_END_ITERATION_SCATTER}" \
            ...
            bash "${RUNNER}"
    preflight_done=1
    echo "[ep32-28-scatter] $(date --iso-8601=seconds) completed ${stage}/${label}"
}
