...
# Original a3b/30b hybrid model with 128 true experts on healthy EP16/EP16.
# Flex uses HybridEP; optimizer stepping is skipped to match the full-size NEP benchmark.
...
#SBATCH --job-name=coreai_comparch_sysarch-healthy.a3b128e-ep16-flex-noopt

set -euo pipefail

REPO_DIR="${REPO_DIR:-/home/darfeen/Megatron-LM}"
ROOT_DIR="${ROOT_DIR:-${REPO_DIR}/slurm_runs/lyris_a3b_30b_original_128e_ep16_healthy_flex_no_optimizer}"
...
NAME="a3b_30b_original_128e_ep16_ep16_flex_hybridep_noopt_i${TRAIN_ITERS}"
...
# Healthy EP16 placement preflight before consuming the 32 training GPUs.
...
placement, _ = compute_nonuniform_ep_expert_placement(128, 16, 16)
assert [len(row) for row in placement] == [8] * 16
assert sorted(expert for row in placement for expert in row) == list(range(128))
slots = _pad_nonuniform_flex_dispatch_slots(
    compute_nonuniform_ep_dispatch_slots(placement, 128), 16, \"hybridep\"
)
assert [len(row) for row in slots] == [8] * 16
assert all(expert is not None for row in slots for expert in row)
print(\"[a3b-ep16-healthy-flex-noopt] placement preflight: PASS\")
'
"

echo "[a3b-ep16-healthy-flex-noopt] $(date --iso-8601=seconds) starting ${NAME}"
timeout --foreground --signal=TERM --kill-after=45s "${CASE_TIMEOUT}" \
    env \
        ...
        MOE_ROUTER_TOPK=6 \
        SEQ_LENGTH=8192 \
        NONUNIFORM_MODE=none \
        NONUNIFORM_EP_TOPOLOGY="8 8" \
...
echo "[a3b-ep16-healthy-flex-noopt] $(date --iso-8601=seconds) training completed"
...
required_config = {
    ...
    "healthy mode": r"nonuniform_mode\s+\.+\s+none",
...
print(
    "[a3b-ep16-healthy-flex-noopt] PASS: original 128-expert a3b/30b architecture, "
    "healthy EP16/EP16, Flex/HybridEP, no optimizer step, "
