...
M=$(getent hosts "$M" | awk '{print $1}' | head -n1)
M=${M:-$(scontrol show hostname $SLURM_NODELIST | head -n1)}

srun --container-image=$I --container-mounts=$W:$W --container-workdir=$W \
     --container-env=NCCL_NVLS_ENABLE=0,NVSHMEM_MAX_TEAMS=512,NVSHMEM_DISABLE_NVLS=1,NCCL_LAUNCH_ORDER_IMPLICIT=1,NVSHMEM_SYMMETRIC_SIZE=4G \
     bash -c "
