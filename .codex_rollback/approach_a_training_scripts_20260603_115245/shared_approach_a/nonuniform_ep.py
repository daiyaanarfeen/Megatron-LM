...
class NonuniformEPNCCLParamAndGradBucketGroup(_ParamAndGradBucketGroup):
    """Approach A: synchronous NCCL gather/allreduce/scatter for nonuniform EP."""
    ...
    def _start_nonuniform_ep_nccl_grad_sync(self):
        cfg = self._nep_runtime_config
        ep_group = cfg['ep_group']
        edp_group = cfg.get('edp_group')
        local_ep_size = cfg['local_ep_size']
        ep_rank = cfg['ep_rank']
        is_edp_eligible = cfg.get(
            'is_edp_eligible',
            ep_rank < cfg.get('min_ep_size', local_ep_size),
        )
        ...
        for bucket in self.buckets:
            ...
            if local_ep_size == 1:
                if edp_group is not None:
                    dist.all_reduce(grad_data, op=reduce_op, group=edp_group)
                continue
            ...
            dist.all_gather(gather_list, grad_data, group=ep_group)

            if is_edp_eligible:
                if edp_group is None:
                    raise RuntimeError(
                        "Nonuniform EP NCCL approach requires runtime_config['edp_group'] "
                        "on EDP-eligible ranks."
                    )
                dist.all_reduce(gather_buf, op=reduce_op, group=edp_group)
            elif edp_group is not None:
                # Match the historical Approach A ordering: follower EP ranks still enter
                # their singleton EDP collective before the EP scatter all-reduce. This
                # keeps NCCL collective sequencing aligned with eligible ranks.
                dist.all_reduce(grad_data, op=reduce_op, group=edp_group)

            if ep_rank != 0:
                gather_buf.zero_()
            dist.all_reduce(gather_buf, op=dist.ReduceOp.SUM, group=ep_group)
    ...
    def start_grad_sync(self, force_all_reduce: Optional[bool] = False):
        ...
        if self._nep_nccl_grad_sync_started:
            return
    ...
    def finish_nep_pre_sync(self, force_all_reduce: Optional[bool] = False):
        """Compatibility hook used by NonuniformEPDistributedDataParallel."""
        return
    ...
    def finish_grad_sync(self, force_all_reduce: Optional[bool] = False):
        ...
        if self.is_first_batch:
            self.start_grad_sync(force_all_reduce=force_all_reduce)
        assert self._nep_nccl_grad_sync_started, (
            f"Communication call has not been issued for this bucket "
            f"({len(self.per_param_grad_ready_counts)}/{len(self.params)} "
            "params have grad available)"
        )
...
class NonuniformEPDistributedDataParallel(DistributedDataParallel):
    ...
    def __init__(
        ...
        if self.nonuniform_ep_config.approach == NonuniformEPApproach.NCCL:
            self.expert_parallel_bucket_groups = wrap_nonuniform_ep_nccl_bucket_groups(
